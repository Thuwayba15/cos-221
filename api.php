...
class HoopAPI {
    ...
    public function registerUser($dbConnection, $name, $surname, $email, $password, $username, $dateOfBirth, $country, $subscription_plan, $accountNumber, $bankName){
        //validate input
        $invalid = false;
        $message = "";
        //echo "helllllooooo";
        ...
        if (!$invalid){
            ...
            $stmt = $dbConnection->prepare("INSERT INTO USERS (username, name, surname, email, password, dateofBirth, country, subscription_plan, api_key, salt) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
            $stmt->bind_param("ssssssssss", $username, $name, $surname, $email, $hashed_password, $dateOfBirth, $country, $subscription_plan, $key, $salt);
            $result = $stmt->execute();
            $userID = $stmt->insert_id;
            $stmt = $dbConnection->prepare("INSERT INTO BANK_ACCOUNT(userID, accountNumber, bankName) VALUES (?, ?, ?)");
            $stmt->bind_param("iss", $userID, $accountNumber, $bankName);
        ...
        if (!$invalid){
            $data = array("apikey" => $key, "message: " => $message, "userID" => $userID);
            $_SESSION['apikey'] = $key;
    ...
    public function login($dbConnection, $input)
    {
        ...
        if (!$invalid){
            //$key = $row["api_key"];
            $data = array("message" => "Login successful", "username" => $row["username"], "name" => $row["name"], "surname" => $row["surname"], "email" => $row["email"], "subscription_plan" => $row["subscription_plan"], "admin" => $row["admin"], "userID" => $row["userID"]);
            //$_SESSION['apikey'] = $key;
    ...
    public function addReview($dbConnection, $input){
        $sql = "INSERT INTO REVIEW (userID, contentID, comment, rating) VALUES (?, ?, ?, ?)";
        $stmt = $dbConnection->prepare($sql);
        $response = [];

        if ($stmt){
            $stmt->bind_param("iisd", $input["userID"], $input["contentID"], $input["comment"], $input["rating"]);
            $completed = $stmt->execute();
            if ($completed){
                $response["status"] = "success";
                $response["timestamp"] = time();
                $response["data"] = array("message" => "Review added successfully");
            }
            else {
                $response["status"] = "error";
                $response["timestamp"] = time();
    ...
    public function deleteContent($dbConnection, $input){
        $sql = "DELETE FROM CONTENT WHERE contentID = ?";
        $stmt = $dbConnection->prepare($sql);
        $response = [];

        if ($stmt){
            $stmt->bind_param("i", $input["contentID"]);
            $completed = $stmt->execute();
            if ($completed){
                $response["status"] = "success";
                $response["timestamp"] = time();
                $response["data"] = array("message" => "Content deleted successfully");
            }
            else {
                $response["status"] = "error";
                $response["timestamp"] = time();
...
if ($data["type"] == "registerUser"){
    ...
    $user->registerUser($dbConnection, $data["name"], $data["surname"], $data["email"], $data["password"], $data["username"], $data["dateOfBirth"], $data["country"], $data["subscription_plan"], $data["accountNumber"], $data["bankName"]);
}

if ($data["type"] == "addReview"){
    $review = HoopAPI::instance();
    $review->addReview($dbConnection, $data);
}

if ($data["type"] == "deleteContent"){
    $delete = HoopAPI::instance();
    $delete->deleteContent($dbConnection, $data);
}
